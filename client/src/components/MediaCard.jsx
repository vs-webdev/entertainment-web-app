...
const MediaCard = ({year, category, rating, title, mediaId, isBookmarked, posterImg, media, setMedia}) => {
  ...
  return (
    <div className="flex flex-col items-start justify-center">
      <div className="rouded h-[210px] lg:h-[174px] w-full rounded-2xl relative mb-2 overflow-hidden cursor-pointer group"
        onMouseEnter={() => setIsHover(true)}
        onMouseLeave={() => setIsHover(false)}
      >
        <div className="absolute inset-0 bg-gray-100 bg-cover bg-center transition-transform duration-500 ease-in-out transform group-hover:scale-103"
          style={{backgroundImage: `url(https://image.tmdb.org/t/p/w500${posterImg})`}}
        ></div>

        <div className={`bg-gray-500/20 inset-0 absolute flex ${isHover ? '' : 'hidden'} items-center justify-center`}>
          <div className='flex items-center w-max gap-2 bg-gray-500 hover:bg-gray-600 px-4 py-2 rounded-full'>
        ...
        <div 
          className='z-10 absolute top-3 right-3 flex items-center justify-center h-8 w-8 cursor-pointer rounded-full bg-gray-500 hover:bg-gray-600'
